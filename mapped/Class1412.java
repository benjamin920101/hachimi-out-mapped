/*
 * Decompiled with CFR 0.152.
 */
package mapped;

import mapped.Class111;
import mapped.Class115;
import mapped.Class1158;
import mapped.Class119;
import mapped.Class12;
import mapped.Class1278;
import mapped.Class1338;
import mapped.Class1345;
import mapped.Class1365;
import mapped.Class1463;
import mapped.Class152;
import mapped.Class1567;
import mapped.Class1578;
import mapped.Class1610;
import mapped.Class162;
import mapped.Class177;
import mapped.Class178;
import mapped.Class2297;
import mapped.Class2298;
import mapped.Class237;
import mapped.Class242;
import mapped.Class257;
import mapped.Class262;
import mapped.NameProtect;
import mapped.Class29;
import mapped.Reach;
import mapped.PacketCanceler;
import mapped.Class3;
import mapped.HoleESP;
import mapped.AntiSpam;
import mapped.InventorySync;
import mapped.AspectRatio;
import mapped.ClickGui;
import mapped.Class3449;
import mapped.ESP;
import mapped.StorageESP;
import mapped.Class3766;
import mapped.Notification;
import mapped.Class3843;
import mapped.Class3879;
import mapped.Class3915;
import mapped.Trajectories;
import mapped.Class40;
import mapped.Class4036;
import mapped.Replenish;
import mapped.Class4104;
import mapped.Surround;
import mapped.AntiPhase;
import mapped.Flatten;
import mapped.AutoCrawlTrap;
import mapped.Class451;
import mapped.Class479;
import mapped.Class546;
import mapped.Class5473;
import mapped.Class5490;
import mapped.Class5541;
import mapped.Class5595;
import mapped.Class560;
import mapped.Class5670;
import mapped.Class571;
import mapped.Class573;
import mapped.Class576;
import mapped.Class578;
import mapped.Class5852;
import mapped.Class5944;
import mapped.Class6;
import mapped.Class607;
import mapped.Class616;
import mapped.Class6161;
import mapped.Class634;
import mapped.Class6461;
import mapped.Class681;
import mapped.Class684;
import mapped.Class686;
import mapped.Class691;
import mapped.Class692;
import mapped.Class698;
import mapped.Class70;
import mapped.Class700;
import mapped.Class703;
import mapped.Class704;
import mapped.Class707;
import mapped.Class734;
import mapped.Class74;
import mapped.Class80;
import mapped.Class92;
import mapped.Class94;

public class Class1412 {
    public static int method14144(int n2, int n3, Class115 class115, Class634 class634) {
        return 940624431 + (n2 & 0xC7EF35D0);
    }

    public static int method14145(int n2, int n3, Class1578 class1578, Class578 class578) {
        int n4 = n2 ^ 0x1A60C0B0;
        return (n3 | ~n4) - ~n4;
    }

    public static int method14146(int n2, int n3, Class571 class571, Class242 class242) {
        int n4 = n2 ^ 0xE5077EB4;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method14147(int n2, int n3, Class257 class257, Class1158 class1158) {
        int n4 = n2;
        return (n4 | 0x113273A1) & (0xEECD8C5E | ~n4);
    }

    public static int method14148(int n2, int n3, Class80 class80, Class177 class177) {
        int n4 = n2;
        return (0x3C1EF530 | ~n4) - ~n4;
    }

    public static int method14149(int n2, int n3, Notification class382, Class1338 class1338) {
        return 433317666 + (n2 & 0xE62C18DD);
    }

    public static int method14150(int n2, int n3, Class691 class691, Class29 class29) {
        int n4 = n2;
        return (0x3D382EA7 | ~n4) - ~n4;
    }

    public static int method14151(int n2, int n3, Class4104 class4104, Class607 class607) {
        return 501691128 + (n2 & 0xE218CD07);
    }

    public static int method14152(int n2, int n3, Class2298 class2298, Class684 class684) {
        return 229052637 + (n2 & 0xF258EF22);
    }

    public static int method14153(int n2, int n3, Class1463 class1463, Class1610 class1610) {
        int n4 = n2;
        return (0x2141E6FE | ~n4) - ~n4;
    }

    public static int method14154(int n2, int n3, Class92 class92, ESP class360) {
        int n4 = n2 ^ 0xA6B5B6A2;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method14155(int n2, int n3, Replenish class408, Class698 class698) {
        int n4 = n2;
        return (0x2CA1115B | ~n4) - ~n4;
    }

    public static int method14156(int n2, int n3, Class479 class479, Class6461 class6461) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xBBDFFF7C) & ~n4);
    }

    public static int method14157(int n2, int n3, Class3 class3, Class111 class111) {
        int n4 = n2 ^ 0xC33694F4;
        return (n3 | ~n4) - ~n4;
    }

    public static int method14158(int n2, int n3, Class5944 class5944, Class6 class6) {
        int n4 = n2;
        return (0x24834B8F | ~n4) - ~n4;
    }

    public static int method14159(int n2, int n3, Class686 class686, Reach class293) {
        int n4 = n2;
        return (0x1CF7ADC2 | ~n4) - ~n4;
    }

    public static int method14160(int n2, int n3, Class3449 class3449, Class1567 class1567) {
        int n4 = n2 ^ 0x159B297F;
        return (n3 | ~n4) - ~n4;
    }

    public static int method14161(int n2, int n3, Class74 class74, Class5490 class5490) {
        int n4 = n2 ^ 0x32010618;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method14162(int n2, int n3, Class178 class178, Class5595 class5595) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x985295BD) & ~n4);
    }

    public static int method14163(int n2, int n3, Class92 class92, Class94 class94) {
        int n4 = n3;
        return n4 + ((n2 ^ 0) & ~n4);
    }

    public static int method14164(int n2, int n3, Class70 class70, Class1365 class1365) {
        int n4 = n2;
        return (0x1B5F35E2 | ~n4) - ~n4;
    }

    public static int method14165(int n2, int n3, Class1158 class1158, Class5595 class5595) {
        int n4 = n2 ^ 0xEE7D485A;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method14166(int n2, int n3, Class704 class704, Class707 class707) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x6B644140) & ~n4);
    }

    public static int method14167(int n2, int n3, Class3843 class3843, Surround class416) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x9AD51588) & ~n4);
    }

    public static int method14168(int n2, int n3, Trajectories class397, Class560 class560) {
        int n4 = n2 ^ 0x14803028;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method14169(int n2, int n3, Class1345 class1345, Class5473 class5473) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xEFFFE2BE) & ~n4);
    }

    public static int method14170(int n2, int n3, Class451 class451, Class4036 class4036) {
        return 124645253 + (n2 & 0xF892107A);
    }

    public static int method14171(int n2, int n3, Class5541 class5541, Class6161 class6161) {
        int n4 = n2;
        return (0x3FC82ED5 | ~n4) - ~n4;
    }

    public static int method14172(int n2, int n3, Class3766 class3766, AntiPhase class417) {
        int n4 = n2;
        return (0x1E8F51D6 | ~n4) - ~n4;
    }

    public static int method14173(int n2, int n3, Class703 class703, Class681 class681) {
        int n4 = n2;
        return (0x2C8C1DB6 | ~n4) - ~n4;
    }

    public static int method14174(int n2, int n3, StorageESP class369, ClickGui class333) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x75F05C06) & ~n4);
    }

    public static int method14175(int n2, int n3, NameProtect class288, Class5852 class5852) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xC0AACC39) & ~n4);
    }

    public static int method14176(int n2, int n3, Class262 class262, Class616 class616) {
        int n4 = n2 ^ 0x30904010;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method14177(int n2, int n3, Class573 class573, HoleESP class315) {
        int n4 = n2;
        return (0xD640536 | ~n4) - ~n4;
    }

    public static int method14178(int n2, int n3, AntiSpam class317, Class40 class40) {
        int n4 = n2;
        return (n4 | 0x254AA940) & (0xDAB556BF | ~n4);
    }

    public static int method14179(int n2, int n3, Class119 class119, Class546 class546) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x7BE86120) & ~n4);
    }

    public static int method14180(int n2, int n3, Class734 class734, Class576 class576) {
        int n4 = n2;
        return (n4 | 0x27291517) & (0xD8D6EAE8 | ~n4);
    }

    public static int method14181(int n2, int n3, Class152 class152, Class5670 class5670) {
        int n4 = n2 ^ 0xD7FB51FC;
        return (n3 | ~n4) - ~n4;
    }

    public static int method14182(int n2, int n3, Flatten class426, Class3915 class3915) {
        int n4 = n2;
        return (0x259FB53C | ~n4) - ~n4;
    }

    public static int method14183(int n2, int n3, Class692 class692, AspectRatio class321) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xE3CBBF6D) & ~n4);
    }

    public static int method14184(int n2, int n3, InventorySync class320, Class700 class700) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x24083608) & ~n4);
    }

    public static int method14185(int n2, int n3, Class3879 class3879, AutoCrawlTrap class430) {
        return n2 ^ 0xEB42B8A;
    }

    public static int method14186(int n2, int n3, PacketCanceler class294, Class2297 class2297) {
        return 437458960 + (n2 & 0xE5ECE7EF);
    }

    public static int method14187(int n2, int n3, Class237 class237, Class162 class162) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x1440002) & ~n4);
    }

    public static int method14188(int n2, int n3, Class1278 class1278, Class12 class12) {
        int n4 = n2;
        return n4 & 0xE1950728 | 0x1E6AF8D7 & ~n4;
    }
}

