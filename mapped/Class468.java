/*
 * Decompiled with CFR 0.152.
 */
package mapped;

import mapped.Class1;
import mapped.Class103;
import mapped.Class105;
import mapped.Class106;
import mapped.Class114;
import mapped.Class1153;
import mapped.Class116;
import mapped.Class1197;
import mapped.Class121;
import mapped.Class1256;
import mapped.Class127;
import mapped.Class132;
import mapped.Class133;
import mapped.Class1345;
import mapped.Class144;
import mapped.Class1447;
import mapped.Class146;
import mapped.Class1463;
import mapped.Class147;
import mapped.Class149;
import mapped.Class1530;
import mapped.Class1536;
import mapped.Class1561;
import mapped.Class161;
import mapped.Class1613;
import mapped.Class1742;
import mapped.Class176;
import mapped.Class1763;
import mapped.Class1789;
import mapped.Class1790;
import mapped.Class189;
import mapped.Class192;
import mapped.Class201;
import mapped.Class207;
import mapped.Class210;
import mapped.Class235;
import mapped.Class241;
import mapped.Class246;
import mapped.Class248;
import mapped.Class249;
import mapped.Class253;
import mapped.Class260;
import mapped.Class268;
import mapped.Class276;
import mapped.Class277;
import mapped.AutoTunnel;
import mapped.Crosshair;
import mapped.Nametags;
import mapped.NoSoundLag;
import mapped.Class31;
import mapped.TrueDurability;
import mapped.Class32;
import mapped.AntiHunger;
import mapped.Class3263;
import mapped.Class3272;
import mapped.Class3332;
import mapped.HUD;
import mapped.TrueSight;
import mapped.AutoReconnect;
import mapped.Class3497;
import mapped.Blink;
import mapped.Class3766;
import mapped.Parkour;
import mapped.PredictPath;
import mapped.XCarry;
import mapped.Class3880;
import mapped.Class3923;
import mapped.NoPacketKick;
import mapped.Class3978;
import mapped.Class4028;
import mapped.Class4036;
import mapped.Zoom;
import mapped.Class4104;
import mapped.Class4134;
import mapped.AutoWeb;
import mapped.AutoCrawlTrap;
import mapped.AutoMine;
import mapped.AutoPotion;
import mapped.FakePlayer;
import mapped.Rotations;
import mapped.Class475;
import mapped.Class520;
import mapped.Class521;
import mapped.Class523;
import mapped.Class529;
import mapped.Class530;
import mapped.Class536;
import mapped.Class538;
import mapped.Class543;
import mapped.Class5550;
import mapped.Class56;
import mapped.Class573;
import mapped.Class580;
import mapped.Class581;
import mapped.Class59;
import mapped.Class598;
import mapped.Class5984;
import mapped.Class6;
import mapped.Class6009;
import mapped.Class601;
import mapped.Class6074;
import mapped.Class610;
import mapped.Class616;
import mapped.Class625;
import mapped.Class6283;
import mapped.Class6339;
import mapped.Class637;
import mapped.Class68;
import mapped.Class682;
import mapped.Class687;
import mapped.Class691;
import mapped.Class696;
import mapped.Class697;
import mapped.Class707;
import mapped.Class708;
import mapped.Class72;
import mapped.Class74;
import mapped.Class76;
import mapped.Class804;
import mapped.Class81;
import mapped.Class85;
import mapped.Class92;
import mapped.Class925;

public class Class468 {
    public static int method10645(int n2, int n3, Class6283 class6283, Class147 class147) {
        return 781763990 + (n2 & 0xD1673A69);
    }

    public static int method10646(int n2, int n3, Class529 class529, Class3332 class3332) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x41022000) & ~n4);
    }

    public static int method10647(int n2, int n3, Class543 class543, Class92 class92) {
        int n4 = n2 ^ 0x84C0804C;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method10648(int n2, int n3, Class85 class85, Class1561 class1561) {
        int n4 = n2 ^ 0xFFFDADEA;
        return (n3 | ~n4) - ~n4;
    }

    public static int method10649(int n2, int n3, Rotations class456, Class610 class610) {
        int n4 = n2 ^ 0x209B2F02;
        return (n3 | ~n4) - ~n4;
    }

    public static int method10650(int n2, int n3, Class3978 class3978, Class1463 class1463) {
        int n4 = n2;
        return (n4 | 0x35DD3C4A) & (0xCA22C3B5 | ~n4);
    }

    public static int method10651(int n2, int n3, Class6009 class6009, AutoWeb class424) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x30040080) & ~n4);
    }

    public static int method10652(int n2, int n3, Class268 class268, Class925 class925) {
        return 510546859 + (n2 & 0xE191AC54);
    }

    public static int method10653(int n2, int n3, Class31 class31, Class210 class210) {
        int n4 = n2;
        return (n4 | 0x3263F657) & (0xCD9C09A8 | ~n4);
    }

    public static int method10654(int n2, int n3, Class253 class253, Class536 class536) {
        return 206572340 + (n2 & 0xF3AFF4CB);
    }

    public static int method10655(int n2, int n3, Class246 class246, Class708 class708) {
        return 1039340494 + (n2 & 0xC20CEC31);
    }

    public static int method10656(int n2, int n3, AutoPotion class441, Class696 class696) {
        int n4 = n2;
        return (0x12E36AF3 | ~n4) - ~n4;
    }

    public static int method10657(int n2, int n3, Class201 class201, AntiHunger class326) {
        int n4 = n2 ^ 0x25492E42;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method10658(int n2, int n3, Class1790 class1790, Class616 class616) {
        int n4 = n2 ^ 0x434368C3;
        return (n3 | ~n4) - ~n4;
    }

    public static int method10659(int n2, int n3, Class521 class521, Class248 class248) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x9248AAE6) & ~n4);
    }

    public static int method10660(int n2, int n3, XCarry class387, AutoCrawlTrap class430) {
        return 213456718 + (n2 & 0xF346E8B1);
    }

    public static int method10661(int n2, int n3, Class538 class538, Class189 class189) {
        int n4 = n2 ^ 0x49A1B220;
        return (n3 | ~n4) - ~n4;
    }

    public static int method10662(int n2, int n3, Class3880 class3880, Class207 class207) {
        int n4 = n2;
        return (0x1364A5E0 | ~n4) - ~n4;
    }

    public static int method10663(int n2, int n3, Class161 class161, Class116 class116) {
        return 92712012 + (n2 & 0xFA7953B3);
    }

    public static int method10664(int n2, int n3, Class4028 class4028, NoPacketKick class394) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x50060A5) & ~n4);
    }

    public static int method10665(int n2, int n3, Class6 class6, Class4134 class4134) {
        return 860219634 + (n2 & 0xCCBA170D);
    }

    public static int method10666(int n2, int n3, Class127 class127, TrueSight class341) {
        int n4 = n2 ^ 0x895D5D66;
        return (n3 | ~n4) - ~n4;
    }

    public static int method10667(int n2, int n3, NoSoundLag class303, Class1 class1) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x33F4C50) & ~n4);
    }

    public static int method10668(int n2, int n3, Class81 class81, Class59 class59) {
        int n4 = n2;
        return (0x1AD30656 | ~n4) - ~n4;
    }

    public static int method10669(int n2, int n3, Class1742 class1742, Class6283 class6283) {
        int n4 = n2 ^ 0x1C000240;
        return (n3 | ~n4) - ~n4;
    }

    public static int method10670(int n2, int n3, Class682 class682, Class6074 class6074) {
        return 626252420 + (n2 & 0xDAAC257B);
    }

    public static int method10671(int n2, int n3, Class523 class523, Class277 class277) {
        return 98268342 + (n2 & 0xFA248B49);
    }

    public static int method10672(int n2, int n3, Class475 class475, Class68 class68) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xF08215B4) & ~n4);
    }

    public static int method10673(int n2, int n3, Class3497 class3497, Nametags class287) {
        return 134837998 + (n2 & 0xF7F68911);
    }

    public static int method10674(int n2, int n3, PredictPath class378, Class1536 class1536) {
        return n2 ^ 0xF137B21A ^ n3;
    }

    public static int method10675(int n2, int n3, Class4134 class4134, Class176 class176) {
        int n4 = n2;
        return (0x1547DB61 | ~n4) - ~n4;
    }

    public static int method10676(int n2, int n3, HUD class339, Class707 class707) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x1B4D260C) & ~n4);
    }

    public static int method10677(int n2, int n3, Class5550 class5550, Class581 class581) {
        int n4 = n2 ^ 0xFD582A00;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method10678(int n2, int n3, Class132 class132, Class276 class276) {
        return 677679068 + (n2 & 0xD79B7023);
    }

    public static int method10679(int n2, int n3, Class114 class114, Class804 class804) {
        return 56968892 + (n2 & 0xFC9AB943);
    }

    public static int method10680(int n2, int n3, Class543 class543, Class1345 class1345) {
        return n2 ^ 0x3A1893FC;
    }

    public static int method10681(int n2, int n3, Class3923 class3923, Blink class376) {
        return 78064884 + (n2 & 0xFB58D30B);
    }

    public static int method10682(int n2, int n3, AutoTunnel class284, Class32 class32) {
        return 811021905 + (n2 & 0xCFA8C9AE);
    }

    public static int method10683(int n2, int n3, Class1530 class1530, Class523 class523) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xCBBBEAE3) & ~n4);
    }

    public static int method10684(int n2, int n3, AutoReconnect class343, Class687 class687) {
        int n4 = n2;
        return n4 & 0xF1BAD11B | 0xE452EE4 & ~n4;
    }

    public static int method10685(int n2, int n3, Class1763 class1763, Class146 class146) {
        int n4 = n2;
        return (0x30F06C3A | ~n4) - ~n4;
    }

    public static int method10686(int n2, int n3, Class72 class72, Class530 class530) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x29E5394B) & ~n4);
    }

    public static int method10687(int n2, int n3, Class536 class536, Class598 class598) {
        int n4 = n2;
        return (0x1573EB48 | ~n4) - ~n4;
    }

    public static int method10688(int n2, int n3, Class207 class207, Class610 class610) {
        return n2 | 0x3AA6CFCD;
    }

    public static int method10689(int n2, int n3, Class235 class235, Class601 class601) {
        int n4 = n2;
        return n4 & 0xDEC82D6C | 0x2137D293 & ~n4;
    }

    public static int method10690(int n2, int n3, AutoMine class435, Class144 class144) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x945AFDD5) & ~n4);
    }

    public static int method10691(int n2, int n3, Class260 class260, Class625 class625) {
        int n4 = n2 ^ 0x8BEF64BE;
        return (n3 | ~n4) - ~n4;
    }

    public static int method10692(int n2, int n3, Class697 class697, Class637 class637) {
        int n4 = n2;
        return (0x366AE651 | ~n4) - ~n4;
    }

    public static int method10693(int n2, int n3, Crosshair class285, Class1197 class1197) {
        int n4 = n2;
        return (0x1BD22DB1 | ~n4) - ~n4;
    }

    public static int method10694(int n2, int n3, Class1789 class1789, Class4036 class4036) {
        return 660056653 + (n2 & 0xD8A855B2);
    }

    public static int method10695(int n2, int n3, Class601 class601, Class3272 class3272) {
        int n4 = n2;
        return (n4 | 0x2BC57692) & (0xD43A896D | ~n4);
    }

    public static int method10696(int n2, int n3, Class106 class106, Class103 class103) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xFFF57BA5) & ~n4);
    }

    public static int method10697(int n2, int n3, FakePlayer class449, Class6339 class6339) {
        int n4 = n2 ^ 0xECBEF1C2;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method10698(int n2, int n3, Class1153 class1153, Class520 class520) {
        int n4 = n2;
        return (n4 | 0xB193042) & (0xF4E6CFBD | ~n4);
    }

    public static int method10699(int n2, int n3, Class121 class121, Class708 class708) {
        int n4 = n2 ^ 0xA0980444;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method10700(int n2, int n3, Class192 class192, Class249 class249) {
        return n2 ^ 0x25FC826;
    }

    public static int method10701(int n2, int n3, Parkour class377, Class5984 class5984) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x64AD66C6) & ~n4);
    }

    public static int method10702(int n2, int n3, Class105 class105, TrueDurability class314) {
        int n4 = n2 ^ 0x8ABFFBF7;
        return (n3 | ~n4) - ~n4;
    }

    public static int method10703(int n2, int n3, Class3766 class3766, Class149 class149) {
        return n2 ^ 0xCF31C3F;
    }

    public static int method10704(int n2, int n3, Class241 class241, Class1256 class1256) {
        int n4 = n2 ^ 0xDFD645D0;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method10705(int n2, int n3, Class133 class133, Class691 class691) {
        int n4 = n2 ^ 0xD8CF8F5F;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method10706(int n2, int n3, Class1613 class1613, Class1536 class1536) {
        return n2 ^ 0x5D0BCD80 ^ n3;
    }

    public static int method10707(int n2, int n3, Class74 class74, Class1447 class1447) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x5908EBD0) & ~n4);
    }

    public static int method10708(int n2, int n3, Class580 class580, Class4104 class4104) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xFEFE56FD) & ~n4);
    }

    public static int method10709(int n2, int n3, Class573 class573, Class3263 class3263) {
        return n2 ^ 0x71F54CEE ^ n3;
    }

    public static int method10710(int n2, int n3, Class76 class76, Zoom class409) {
        int n4 = n2;
        return n4 & 0xFEA80F9A | 0x157F065 & ~n4;
    }

    public static int method10711(int n2, int n3, Class3263 class3263, Class56 class56) {
        return 235255037 + (n2 & 0xF1FA4B02);
    }
}

