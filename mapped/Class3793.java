/*
 * Decompiled with CFR 0.152.
 */
package mapped;

import mapped.Class106;
import mapped.Class129;
import mapped.Class130;
import mapped.Class145;
import mapped.Class16;
import mapped.Class162;
import mapped.Class166;
import mapped.Class1718;
import mapped.Class191;
import mapped.Class215;
import mapped.Class248;
import mapped.Class264;
import mapped.Class265;
import mapped.Flight;
import mapped.Class29;
import mapped.Class34;
import mapped.Ping;
import mapped.TridentTweaks;
import mapped.Class3814;
import mapped.Class3913;
import mapped.Class3957;
import mapped.Class4132;
import mapped.Class4218;
import mapped.Follower;
import mapped.Phase;
import mapped.Server;
import mapped.Class536;
import mapped.Class547;
import mapped.Class5496;
import mapped.Class58;
import mapped.Class607;
import mapped.Class6257;
import mapped.Class6343;
import mapped.Class691;
import mapped.Class79;

public class Class3793 {
    public static int method18368(int n2, int n3, Class3913 class3913, Ping class359) {
        int n4 = n2;
        return (0x3990325C | ~n4) - ~n4;
    }

    public static int method18369(int n2, int n3, Class130 class130, Class215 class215) {
        return n2 ^ 0x50303C3;
    }

    public static int method18370(int n2, int n3, Flight class280, Class1718 class1718) {
        return n2 ^ 0xE3BE7FAF ^ n3;
    }

    public static int method18371(int n2, int n3, Class265 class265, Class4218 class4218) {
        int n4 = n2;
        return (n4 | 0x35238A54) & (0xCADC75AB | ~n4);
    }

    public static int method18372(int n2, int n3, Class6257 class6257, Phase class445) {
        int n4 = n2;
        return n4 & 0xCDFE21AF | 0x3201DE50 & ~n4;
    }

    public static int method18373(int n2, int n3, Class3957 class3957, Class29 class29) {
        return 721736138 + (n2 & 0xD4FB2E35);
    }

    public static int method18374(int n2, int n3, Class162 class162, Class5496 class5496) {
        int n4 = n2 ^ 0x6B663230;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method18375(int n2, int n3, Class691 class691, Class248 class248) {
        int n4 = n2;
        return n4 & 0xE7431ABF | 0x18BCE540 & ~n4;
    }

    public static int method18376(int n2, int n3, Class4132 class4132, Class264 class264) {
        int n4 = n2 ^ 0xBB2F8F4B;
        return (n3 | ~n4) - ~n4;
    }

    public static int method18377(int n2, int n3, Class129 class129, Class607 class607) {
        int n4 = n2;
        return (0x259A2449 | ~n4) - ~n4;
    }

    public static int method18378(int n2, int n3, Class6343 class6343, Class191 class191) {
        int n4 = n2 ^ 0x57370BB4;
        return (n3 | ~n4) - ~n4;
    }

    public static int method18379(int n2, int n3, Class191 class191, Class79 class79) {
        int n4 = n2 ^ 0xE7FFABF5;
        return (n3 | ~n4) - ~n4;
    }

    public static int method18380(int n2, int n3, Class58 class58, Server class453) {
        return 445632787 + (n2 & 0xE5702EEC);
    }

    public static int method18381(int n2, int n3, Class3814 class3814, TridentTweaks class363) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xBAAEA000) & ~n4);
    }

    public static int method18382(int n2, int n3, Class16 class16, Class536 class536) {
        return 1013819067 + (n2 & 0xC3925944);
    }

    public static int method18383(int n2, int n3, Class145 class145, Class166 class166) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xD6D522A0) & ~n4);
    }

    public static int method18384(int n2, int n3, Follower class423, Class547 class547) {
        int n4 = n2 ^ 0xC780CD9F;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method18385(int n2, int n3, Class106 class106, Class34 class34) {
        int n4 = n2;
        return (n4 | 0x2FC52E89) & (0xD03AD176 | ~n4);
    }
}

