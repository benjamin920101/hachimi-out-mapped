/*
 * Decompiled with CFR 0.152.
 */
package mapped;

import mapped.Class1011;
import mapped.Class104;
import mapped.Class107;
import mapped.Class108;
import mapped.Class11;
import mapped.Class110;
import mapped.Class1104;
import mapped.Class1226;
import mapped.Class127;
import mapped.Class131;
import mapped.Class1365;
import mapped.Class145;
import mapped.Class1450;
import mapped.Class146;
import mapped.Class148;
import mapped.Class150;
import mapped.Class1503;
import mapped.Class158;
import mapped.Class165;
import mapped.Class1659;
import mapped.Class1706;
import mapped.Class1750;
import mapped.Class176;
import mapped.Class1763;
import mapped.Class1790;
import mapped.Class19;
import mapped.Class192;
import mapped.Class194;
import mapped.Class195;
import mapped.Class196;
import mapped.Class199;
import mapped.Class200;
import mapped.Class202;
import mapped.Class213;
import mapped.Class23;
import mapped.Class233;
import mapped.Class235;
import mapped.Class239;
import mapped.Class241;
import mapped.Class246;
import mapped.Class255;
import mapped.Class262;
import mapped.Class266;
import mapped.Class285;
import mapped.Class286;
import mapped.Class311;
import mapped.Class317;
import mapped.Class322;
import mapped.Class3293;
import mapped.Class340;
import mapped.Class3419;
import mapped.Class3426;
import mapped.Class3497;
import mapped.Class350;
import mapped.Class3511;
import mapped.Class36;
import mapped.Class3684;
import mapped.Class369;
import mapped.Class370;
import mapped.Class381;
import mapped.Class3817;
import mapped.Class3843;
import mapped.Class386;
import mapped.Class3880;
import mapped.Class3935;
import mapped.Class3952;
import mapped.Class40;
import mapped.Class4081;
import mapped.Class423;
import mapped.Class429;
import mapped.Class434;
import mapped.Class442;
import mapped.Class448;
import mapped.Class453;
import mapped.Class480;
import mapped.Class4971;
import mapped.Class520;
import mapped.Class523;
import mapped.Class53;
import mapped.Class537;
import mapped.Class5460;
import mapped.Class548;
import mapped.Class5541;
import mapped.Class5550;
import mapped.Class5587;
import mapped.Class5648;
import mapped.Class5649;
import mapped.Class5655;
import mapped.Class5694;
import mapped.Class5760;
import mapped.Class5829;
import mapped.Class591;
import mapped.Class6074;
import mapped.Class6154;
import mapped.Class626;
import mapped.Class64;
import mapped.Class6408;
import mapped.Class684;
import mapped.Class69;
import mapped.Class704;
import mapped.Class705;
import mapped.Class713;
import mapped.Class716;
import mapped.Class784;
import mapped.Class804;
import mapped.Class81;
import mapped.Class84;
import mapped.Class873;

public class Class4074 {
    public static int method20264(int n2, int n3, Class176 class176, Class36 class36) {
        return n2 ^ 0x3F09DC63;
    }

    public static int method20265(int n2, int n3, Class5587 class5587, Class369 class369) {
        return n2 ^ 0x712DF8D7 ^ n3;
    }

    public static int method20266(int n2, int n3, Class23 class23, Class784 class784) {
        int n4 = n2 ^ 0xA1DA7417;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20267(int n2, int n3, Class200 class200, Class108 class108) {
        int n4 = n2;
        return (0x29BD17B3 | ~n4) - ~n4;
    }

    public static int method20268(int n2, int n3, Class626 class626, Class64 class64) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x8514A21) & ~n4);
    }

    public static int method20269(int n2, int n3, Class19 class19, Class107 class107) {
        int n4 = n2;
        return (0x18A3B71E | ~n4) - ~n4;
    }

    public static int method20270(int n2, int n3, Class5760 class5760, Class1365 class1365) {
        int n4 = n2;
        return (0x38623B89 | ~n4) - ~n4;
    }

    public static int method20271(int n2, int n3, Class235 class235, Class1659 class1659) {
        int n4 = n2;
        return (0x2510B971 | ~n4) - ~n4;
    }

    public static int method20272(int n2, int n3, Class480 class480, Class213 class213) {
        int n4 = n2 ^ 0x1D36E6E5;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20273(int n2, int n3, Class684 class684, Class165 class165) {
        int n4 = n2;
        return (0x13E8AEAF | ~n4) - ~n4;
    }

    public static int method20274(int n2, int n3, Class150 class150, Class286 class286) {
        int n4 = n2 ^ 0xBFFBE6DC;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20275(int n2, int n3, Class145 class145, Class1790 class1790) {
        return 154573858 + (n2 & 0xF6C963DD);
    }

    public static int method20276(int n2, int n3, Class195 class195, Class591 class591) {
        int n4 = n2 ^ 0xE5CAF5D5;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20277(int n2, int n3, Class1503 class1503, Class3511 class3511) {
        int n4 = n2;
        return (n4 | 0x294AF562) & (0xD6B50A9D | ~n4);
    }

    public static int method20278(int n2, int n3, Class448 class448, Class1750 class1750) {
        return 257234763 + (n2 & 0xF0AAE8B4);
    }

    public static int method20279(int n2, int n3, Class110 class110, Class131 class131) {
        int n4 = n2 ^ 0xF7A1FFBB;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20280(int n2, int n3, Class548 class548, Class127 class127) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xDFAFEF69) & ~n4);
    }

    public static int method20281(int n2, int n3, Class148 class148, Class262 class262) {
        int n4 = n2;
        return (0x19A99B0 | ~n4) - ~n4;
    }

    public static int method20282(int n2, int n3, Class523 class523, Class81 class81) {
        int n4 = n2 ^ 0x4A950E96;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20283(int n2, int n3, Class317 class317, Class69 class69) {
        int n4 = n2;
        return (n4 | 0x419E407) & (0xFBE61BF8 | ~n4);
    }

    public static int method20284(int n2, int n3, Class246 class246, Class3419 class3419) {
        int n4 = n2 ^ 0xF5FFFEB9;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20285(int n2, int n3, Class1706 class1706, Class239 class239) {
        int n4 = n2 ^ 0x954F5D5A;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method20286(int n2, int n3, Class713 class713, Class6408 class6408) {
        int n4 = n2;
        return (0x2A60A4D7 | ~n4) - ~n4;
    }

    public static int method20287(int n2, int n3, Class429 class429, Class1763 class1763) {
        return 694856177 + (n2 & 0xD695560E);
    }

    public static int method20288(int n2, int n3, Class285 class285, Class311 class311) {
        int n4 = n2 ^ 0xF3E9C9EF;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method20289(int n2, int n3, Class84 class84, Class194 class194) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xFEFFDFFF) & ~n4);
    }

    public static int method20290(int n2, int n3, Class241 class241, Class340 class340) {
        return 8400760 + (n2 & 0xFF7FD087);
    }

    public static int method20291(int n2, int n3, Class213 class213, Class716 class716) {
        int n4 = n2 ^ 0x4C44244;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20292(int n2, int n3, Class213 class213, Class255 class255) {
        int n4 = n2;
        return (0x2465BC0B | ~n4) - ~n4;
    }

    public static int method20293(int n2, int n3, Class704 class704, Class684 class684) {
        int n4 = n2;
        return n4 & 0xC143BBDC | 0x3EBC4423 & ~n4;
    }

    public static int method20294(int n2, int n3, Class196 class196, Class3880 class3880) {
        int n4 = n2 ^ 0xFF5F61BF;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method20295(int n2, int n3, Class1365 class1365, Class6154 class6154) {
        int n4 = n2 ^ 0x51E211F;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20296(int n2, int n3, Class434 class434, Class53 class53) {
        int n4 = n2;
        return (0x26EBA788 | ~n4) - ~n4;
    }

    public static int method20297(int n2, int n3, Class1104 class1104, Class3419 class3419) {
        int n4 = n2 ^ 0xAD0D0E74;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method20298(int n2, int n3, Class3952 class3952, Class5829 class5829) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xBB97C4E4) & ~n4);
    }

    public static int method20299(int n2, int n3, Class4971 class4971, Class266 class266) {
        int n4 = n2;
        return (0x21941EA1 | ~n4) - ~n4;
    }

    public static int method20300(int n2, int n3, Class146 class146, Class40 class40) {
        return n2 ^ 0x908080B4 ^ n3;
    }

    public static int method20301(int n2, int n3, Class453 class453, Class3684 class3684) {
        int n4 = n2 ^ 0x88001006;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20302(int n2, int n3, Class381 class381, Class202 class202) {
        int n4 = n2;
        return (0x2D4DA2FE | ~n4) - ~n4;
    }

    public static int method20303(int n2, int n3, Class423 class423, Class262 class262) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xD6F6EEEF) & ~n4);
    }

    public static int method20304(int n2, int n3, Class6074 class6074, Class317 class317) {
        int n4 = n2 ^ 0xB23D2BA1;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20305(int n2, int n3, Class104 class104, Class873 class873) {
        return 182722136 + (n2 & 0xF51BE1A7);
    }

    public static int method20306(int n2, int n3, Class3843 class3843, Class1226 class1226) {
        return n2 ^ 0x273100D ^ n3;
    }

    public static int method20307(int n2, int n3, Class784 class784, Class370 class370) {
        int n4 = n2;
        return (n4 | 0x12C28F61) & (0xED3D709E | ~n4);
    }

    public static int method20308(int n2, int n3, Class537 class537, Class3293 class3293) {
        return n2 ^ 0x50C6F71F ^ n3;
    }

    public static int method20309(int n2, int n3, Class11 class11, Class3935 class3935) {
        int n4 = n2;
        return (n4 | 0x28F979FC) & (0xD7068603 | ~n4);
    }

    public static int method20310(int n2, int n3, Class4081 class4081, Class5648 class5648) {
        int n4 = n2 ^ 0x90413106;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20311(int n2, int n3, Class192 class192, Class5541 class5541) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x2A473E74) & ~n4);
    }

    public static int method20312(int n2, int n3, Class322 class322, Class520 class520) {
        return 791738781 + (n2 & 0xD0CF0662);
    }

    public static int method20313(int n2, int n3, Class5460 class5460, Class442 class442) {
        return 783275512 + (n2 & 0xD1502A07);
    }

    public static int method20314(int n2, int n3, Class5550 class5550, Class233 class233) {
        int n4 = n2;
        return (0xCAD107D | ~n4) - ~n4;
    }

    public static int method20315(int n2, int n3, Class705 class705, Class5694 class5694) {
        return 870931279 + (n2 & 0xCC16A4B0);
    }

    public static int method20316(int n2, int n3, Class3497 class3497, Class1011 class1011) {
        int n4 = n2 ^ 0x12400100;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20317(int n2, int n3, Class3426 class3426, Class5655 class5655) {
        int n4 = n2;
        return (0x545796D | ~n4) - ~n4;
    }

    public static int method20318(int n2, int n3, Class239 class239, Class1450 class1450) {
        return n2 ^ 0x63FFF2A;
    }

    public static int method20319(int n2, int n3, Class199 class199, Class350 class350) {
        return n2 ^ 0xDA342404 | n3;
    }

    public static int method20320(int n2, int n3, Class386 class386, Class5649 class5649) {
        int n4 = n2 ^ 0x79AE6E7E;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20321(int n2, int n3, Class3817 class3817, Class804 class804) {
        int n4 = n2;
        return (0x1A9603E8 | ~n4) - ~n4;
    }

    public static int method20322(int n2, int n3, Class713 class713, Class158 class158) {
        int n4 = n2 ^ 0x2413FB11;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }
}

