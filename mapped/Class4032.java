/*
 * Decompiled with CFR 0.152.
 */
package mapped;

import mapped.Class10;
import mapped.Class1004;
import mapped.Class109;
import mapped.Class111;
import mapped.Class115;
import mapped.Class118;
import mapped.Class134;
import mapped.Class1345;
import mapped.Class135;
import mapped.Class1405;
import mapped.Class141;
import mapped.Class1450;
import mapped.Class1485;
import mapped.Class149;
import mapped.Class1503;
import mapped.Class155;
import mapped.Class156;
import mapped.Class1578;
import mapped.Class160;
import mapped.Class1661;
import mapped.Class1662;
import mapped.Class1664;
import mapped.Class167;
import mapped.Class174;
import mapped.Class18;
import mapped.Class1804;
import mapped.Class1807;
import mapped.Class184;
import mapped.Class197;
import mapped.Class2;
import mapped.Class203;
import mapped.Class212;
import mapped.Class217;
import mapped.Class218;
import mapped.Class228;
import mapped.Class23;
import mapped.Class233;
import mapped.Class239;
import mapped.Class241;
import mapped.Class248;
import mapped.Class252;
import mapped.Class274;
import mapped.Class28;
import mapped.AutoTunnel;
import mapped.Class30;
import mapped.AutoLog;
import mapped.AutoWalk;
import mapped.Class32;
import mapped.Class3221;
import mapped.Class3285;
import mapped.Class33;
import mapped.AutoFish;
import mapped.Class3323;
import mapped.Class3377;
import mapped.HUD;
import mapped.AutoReconnect;
import mapped.GhostHand;
import mapped.AutoTool;
import mapped.ElytraTweaks;
import mapped.PenisBuilder;
import mapped.BetterChat;
import mapped.AutoEat;
import mapped.SafeWalk;
import mapped.PMSound;
import mapped.Class3730;
import mapped.Class38;
import mapped.Class3817;
import mapped.Class3843;
import mapped.FastPlace;
import mapped.Class3868;
import mapped.NoGlitch;
import mapped.Class3923;
import mapped.Class3933;
import mapped.Fullbright;
import mapped.Class3957;
import mapped.PhantomView;
import mapped.Class3979;
import mapped.ArmorAlert;
import mapped.Class4113;
import mapped.Class4122;
import mapped.Class4134;
import mapped.Class414;
import mapped.PearlBlocker;
import mapped.PistonCrystal;
import mapped.Class4244;
import mapped.SelfTrap;
import mapped.Phase;
import mapped.Class477;
import mapped.Class479;
import mapped.Class52;
import mapped.Class534;
import mapped.Class5394;
import mapped.Class542;
import mapped.Class5460;
import mapped.Class550;
import mapped.Class5655;
import mapped.Class570;
import mapped.Class571;
import mapped.Class573;
import mapped.Class575;
import mapped.Class5759;
import mapped.Class576;
import mapped.Class5760;
import mapped.Class583;
import mapped.Class585;
import mapped.Class591;
import mapped.Class5931;
import mapped.Class600;
import mapped.Class6017;
import mapped.Class6074;
import mapped.Class61;
import mapped.Class6159;
import mapped.Class616;
import mapped.Class6188;
import mapped.Class625;
import mapped.Class6253;
import mapped.Class6257;
import mapped.Class6283;
import mapped.Class6286;
import mapped.Class629;
import mapped.Class6316;
import mapped.Class64;
import mapped.Class6408;
import mapped.Class682;
import mapped.Class687;
import mapped.Class688;
import mapped.Class697;
import mapped.Class699;
import mapped.Class7;
import mapped.Class70;
import mapped.Class700;
import mapped.Class702;
import mapped.Class713;
import mapped.Class79;
import mapped.Class9;
import mapped.Class906;
import mapped.Class909;
import mapped.Class97;

public class Class4032 {
    public static int method20009(int n2, int n3, Class629 class629, Class233 class233) {
        int n4 = n2;
        return (n4 | 0x2BF40041) & (0xD40BFFBE | ~n4);
    }

    public static int method20010(int n2, int n3, Class3730 class3730, Class141 class141) {
        return 244079846 + (n2 & 0xF173A319);
    }

    public static int method20011(int n2, int n3, PhantomView class396, Class542 class542) {
        return 741998210 + (n2 & 0xD3C6017D);
    }

    public static int method20012(int n2, int n3, Class1405 class1405, Class212 class212) {
        int n4 = n2;
        return (0x10ECD215 | ~n4) - ~n4;
    }

    public static int method20013(int n2, int n3, Class1345 class1345, HUD class339) {
        int n4 = n2;
        return (0x3469BC40 | ~n4) - ~n4;
    }

    public static int method20014(int n2, int n3, Class5931 class5931, Class241 class241) {
        return 1038673629 + (n2 & 0xC2171922);
    }

    public static int method20015(int n2, int n3, SelfTrap class432, Class682 class682) {
        int n4 = n2 ^ 0xFEEEE392;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20016(int n2, int n3, Class550 class550, Class174 class174) {
        int n4 = n2;
        return n4 & 0xCC347D2A | 0x33CB82D5 & ~n4;
    }

    public static int method20017(int n2, int n3, Class197 class197, Class6283 class6283) {
        int n4 = n2 ^ 0x6FAFFEA7;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20018(int n2, int n3, PearlBlocker class415, Class239 class239) {
        return n2 ^ 0xC24A922;
    }

    public static int method20019(int n2, int n3, Class600 class600, Class570 class570) {
        int n4 = n2;
        return n4 & 0xDF7C1810 | 0x2083E7EF & ~n4;
    }

    public static int method20020(int n2, int n3, Class70 class70, Class3285 class3285) {
        int n4 = n2;
        return (0x3035B873 | ~n4) - ~n4;
    }

    public static int method20021(int n2, int n3, AutoTunnel class284, Class149 class149) {
        int n4 = n2;
        return (0x24ED60FF | ~n4) - ~n4;
    }

    public static int method20022(int n2, int n3, Class1804 class1804, Class6074 class6074) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xEEFABE7F) & ~n4);
    }

    public static int method20023(int n2, int n3, Class6316 class6316, Class33 class33) {
        return 375367869 + (n2 & 0xE9A05742);
    }

    public static int method20024(int n2, int n3, Class699 class699, Class5655 class5655) {
        int n4 = n2 ^ 0x8CC38F7C;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20025(int n2, int n3, Class6253 class6253, Class248 class248) {
        int n4 = n2 ^ 0xE33C6BA0;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20026(int n2, int n3, Class109 class109, Class1485 class1485) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xC000B88) & ~n4);
    }

    public static int method20027(int n2, int n3, Class61 class61, Class2 class2) {
        return 12817566 + (n2 & 0xFF3C6B61);
    }

    public static int method20028(int n2, int n3, AutoLog class311, Class3957 class3957) {
        int n4 = n2;
        return (0x39A7114C | ~n4) - ~n4;
    }

    public static int method20029(int n2, int n3, Class4244 class4244, Class134 class134) {
        int n4 = n2 ^ 0x4734B01C;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20030(int n2, int n3, Class6159 class6159, Class7 class7) {
        int n4 = n2 ^ 0x84605100;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20031(int n2, int n3, Class18 class18, Class217 class217) {
        return n2 ^ 0xC250B436 ^ n3;
    }

    public static int method20032(int n2, int n3, Class700 class700, Class10 class10) {
        int n4 = n2 ^ 0xD3BBB102;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20033(int n2, int n3, Class3933 class3933, Class197 class197) {
        return n2 ^ 0xAC22842C | n3;
    }

    public static int method20034(int n2, int n3, Class79 class79, Class274 class274) {
        return n2 ^ 0x28255164;
    }

    public static int method20035(int n2, int n3, GhostHand class346, Class697 class697) {
        int n4 = n2 ^ 0x201000;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20036(int n2, int n3, Class156 class156, Class4122 class4122) {
        return n2 ^ 0xE8BFFBFF ^ n3;
    }

    public static int method20037(int n2, int n3, Class576 class576, Class9 class9) {
        int n4 = n2 ^ 0xCF76A251;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20038(int n2, int n3, Class3377 class3377, Class228 class228) {
        int n4 = n2 ^ 0xAA8FF71D;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20039(int n2, int n3, AutoLog class311, Class6408 class6408) {
        int n4 = n2;
        return (0x2BF3F5B3 | ~n4) - ~n4;
    }

    public static int method20040(int n2, int n3, Class3868 class3868, Class3817 class3817) {
        int n4 = n2;
        return n4 & 0xF9A29ED6 | 0x65D6129 & ~n4;
    }

    public static int method20041(int n2, int n3, Class3979 class3979, Class573 class573) {
        int n4 = n2 ^ 0x19AB787E;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method20042(int n2, int n3, Class97 class97, Class591 class591) {
        int n4 = n2 ^ 0xF247698;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20043(int n2, int n3, Class629 class629, Class6017 class6017) {
        return 165157670 + (n2 & 0xF627E4D9);
    }

    public static int method20044(int n2, int n3, Class241 class241, Class575 class575) {
        int n4 = n2 ^ 0x48100109;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20045(int n2, int n3, Class5760 class5760, Class414 class414) {
        return 794598153 + (n2 & 0xD0A364F6);
    }

    public static int method20046(int n2, int n3, AutoReconnect class343, Class3221 class3221) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xEEF7F2F7) & ~n4);
    }

    public static int method20047(int n2, int n3, Class28 class28, Class3843 class3843) {
        int n4 = n2 ^ 0xD73B96B0;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20048(int n2, int n3, Class239 class239, Class5759 class5759) {
        int n4 = n2;
        return (0x92EA37E | ~n4) - ~n4;
    }

    public static int method20049(int n2, int n3, Class687 class687, Class218 class218) {
        int n4 = n2;
        return (n4 | 0xA352895) & (0xF5CAD76A | ~n4);
    }

    public static int method20050(int n2, int n3, SafeWalk class366, Class625 class625) {
        int n4 = n2;
        return (n4 | 0x2D3BB0EE) & (0xD2C44F11 | ~n4);
    }

    public static int method20051(int n2, int n3, Class64 class64, Class1578 class1578) {
        int n4 = n2 ^ 0x8340394E;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20052(int n2, int n3, PenisBuilder class361, Class115 class115) {
        int n4 = n2 ^ 0x43C06E05;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20053(int n2, int n3, NoGlitch class391, Class6188 class6188) {
        return 300102839 + (n2 & 0xEE1CCB48);
    }

    public static int method20054(int n2, int n3, ElytraTweaks class354, AutoEat class364) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x443976E0) & ~n4);
    }

    public static int method20055(int n2, int n3, Class203 class203, Class1503 class1503) {
        int n4 = n2 ^ 0x610D6100;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20056(int n2, int n3, Class6257 class6257, BetterChat class362) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x90575311) & ~n4);
    }

    public static int method20057(int n2, int n3, AutoTool class347, Class141 class141) {
        return n2 ^ 0x2381D377;
    }

    public static int method20058(int n2, int n3, Class1004 class1004, Class583 class583) {
        return 461309296 + (n2 & 0xE480FA8F);
    }

    public static int method20059(int n2, int n3, Class909 class909, Class1345 class1345) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x5A8582A0) & ~n4);
    }

    public static int method20060(int n2, int n3, Class135 class135, AutoWalk class313) {
        return n2 ^ 0x6078F3BE ^ n3;
    }

    public static int method20061(int n2, int n3, PMSound class370, Phase class445) {
        return 129711958 + (n2 & 0xF844C0A9);
    }

    public static int method20062(int n2, int n3, Class3923 class3923, Class32 class32) {
        int n4 = n2;
        return (0x2D30D076 | ~n4) - ~n4;
    }

    public static int method20063(int n2, int n3, Class616 class616, Class3323 class3323) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xB04305C8) & ~n4);
    }

    public static int method20064(int n2, int n3, Class713 class713, Class228 class228) {
        int n4 = n2 ^ 0x576EF076;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20065(int n2, int n3, Class2 class2, Class479 class479) {
        int n4 = n2;
        return (0xA6891 | ~n4) - ~n4;
    }

    public static int method20066(int n2, int n3, Class252 class252, PhantomView class396) {
        int n4 = n2 ^ 0xB40CD43A;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20067(int n2, int n3, Class23 class23, Class585 class585) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xC1610528) & ~n4);
    }

    public static int method20068(int n2, int n3, Class155 class155, Class160 class160) {
        int n4 = n2 ^ 0xC531A022;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20069(int n2, int n3, Class1662 class1662, Class30 class30) {
        int n4 = n2;
        return (n4 | 0xE1F82C2) & (0xF1E07D3D | ~n4);
    }

    public static int method20070(int n2, int n3, Class4113 class4113, Fullbright class395) {
        return 140243133 + (n2 & 0xF7A40F42);
    }

    public static int method20071(int n2, int n3, Class6253 class6253, Class571 class571) {
        int n4 = n2;
        return (n4 | 0x1A5434D9) & (0xE5ABCB26 | ~n4);
    }

    public static int method20072(int n2, int n3, Class6286 class6286, Class5394 class5394) {
        return n2 ^ 0x27357085;
    }

    public static int method20073(int n2, int n3, Class1664 class1664, Class167 class167) {
        int n4 = n2 ^ 0x9EEE00;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20074(int n2, int n3, FastPlace class386, Class111 class111) {
        return 76608949 + (n2 & 0xFB6F0A4A);
    }

    public static int method20075(int n2, int n3, Class1450 class1450, AutoFish class330) {
        int n4 = n2;
        return (0x3F5BEC2F | ~n4) - ~n4;
    }

    public static int method20076(int n2, int n3, Class702 class702, Class1807 class1807) {
        int n4 = n2 ^ 0x1020148A;
        return (n3 | ~n4) - ~n4;
    }

    public static int method20077(int n2, int n3, Class4122 class4122, Class477 class477) {
        int n4 = n2 ^ 0x9000000;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method20078(int n2, int n3, ArmorAlert class398, Class38 class38) {
        return 148896083 + (n2 & 0xF72006AC);
    }

    public static int method20079(int n2, int n3, Class1661 class1661, Class184 class184) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x802B2E11) & ~n4);
    }

    public static int method20080(int n2, int n3, Class688 class688, Class5460 class5460) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xD8865B88) & ~n4);
    }

    public static int method20081(int n2, int n3, Class52 class52, Class534 class534) {
        int n4 = n2;
        return (n4 | 0x6DDA7B2) & (0xF922584D | ~n4);
    }

    public static int method20082(int n2, int n3, Class118 class118, Class4134 class4134) {
        int n4 = n2;
        return (0x1BE44DD0 | ~n4) - ~n4;
    }

    public static int method20083(int n2, int n3, Class906 class906, PistonCrystal class421) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x30893512) & ~n4);
    }
}

