/*
 * Decompiled with CFR 0.152.
 */
package mapped;

import mapped.Class114;
import mapped.Class171;
import mapped.Class190;
import mapped.Class229;
import mapped.Class231;
import mapped.Class234;
import mapped.Class248;
import mapped.Class261;
import mapped.Class268;
import mapped.Class27;
import mapped.Class277;
import mapped.AutoArmor;
import mapped.Class31;
import mapped.Class3209;
import mapped.Class3293;
import mapped.AutoFish;
import mapped.Shulkerception;
import mapped.Class3766;
import mapped.XCarry;
import mapped.ChorusControl;
import mapped.Class3933;
import mapped.Class3978;
import mapped.Class3979;
import mapped.NoMineAnimation;
import mapped.Class4036;
import mapped.MultiTask;
import mapped.Class411;
import mapped.AutoCrystal;
import mapped.AntiCheat;
import mapped.Class538;
import mapped.Class544;
import mapped.Class5457;
import mapped.Class5496;
import mapped.Class560;
import mapped.Class57;
import mapped.Class5758;
import mapped.Class599;
import mapped.Class6134;
import mapped.Class619;
import mapped.Class632;
import mapped.Class67;
import mapped.Class682;
import mapped.Class704;
import mapped.Class715;
import mapped.Class85;
import mapped.Class92;
import mapped.Class93;

public class Class3937 {
    public static int method19401(int n2, int n3, ChorusControl class389, Class229 class229) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x6FFBB97D) & ~n4);
    }

    public static int method19402(int n2, int n3, Class4036 class4036, Class6134 class6134) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xEDBC6C00) & ~n4);
    }

    public static int method19403(int n2, int n3, Class538 class538, Class5758 class5758) {
        int n4 = n2;
        return (0xFA1A595 | ~n4) - ~n4;
    }

    public static int method19404(int n2, int n3, Class85 class85, Class248 class248) {
        int n4 = n2 ^ 0x19524757;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19405(int n2, int n3, Class544 class544, Class599 class599) {
        int n4 = n2 ^ 0x8FE5FFFF;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19406(int n2, int n3, Class619 class619, AntiCheat class455) {
        int n4 = n2 ^ 0x74EEFB40;
        int n5 = n3;
        return n4 & ~n5 | n5 & ~n4;
    }

    public static int method19407(int n2, int n3, Class27 class27, Class704 class704) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x8D819B30) & ~n4);
    }

    public static int method19408(int n2, int n3, Class93 class93, Class3933 class3933) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xC1C47DB7) & ~n4);
    }

    public static int method19409(int n2, int n3, AutoArmor class279, Class261 class261) {
        int n4 = n2;
        return (0x23121B55 | ~n4) - ~n4;
    }

    public static int method19410(int n2, int n3, Class234 class234, Class632 class632) {
        int n4 = n2;
        return n4 & 0xCF399BE6 | 0x30C66419 & ~n4;
    }

    public static int method19411(int n2, int n3, Class114 class114, Class715 class715) {
        int n4 = n2;
        return (0x7A4E29 | ~n4) - ~n4;
    }

    public static int method19412(int n2, int n3, Class3978 class3978, Class92 class92) {
        return 165327020 + (n2 & 0xF6254F53);
    }

    public static int method19413(int n2, int n3, Class560 class560, Class268 class268) {
        return 227450367 + (n2 & 0xF2716200);
    }

    public static int method19414(int n2, int n3, Class57 class57, Class3979 class3979) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xE0021440) & ~n4);
    }

    public static int method19415(int n2, int n3, NoMineAnimation class400, Class67 class67) {
        int n4 = n2 ^ 0xF5824783;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19416(int n2, int n3, Class3293 class3293, Class3766 class3766) {
        return n2 ^ 0xEA7FF7F3 ^ n3;
    }

    public static int method19417(int n2, int n3, AutoFish class330, Class5496 class5496) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x85D052F0) & ~n4);
    }

    public static int method19418(int n2, int n3, Class231 class231, Class171 class171) {
        return 88803308 + (n2 & 0xFAB4F813);
    }

    public static int method19419(int n2, int n3, Class411 class411, XCarry class387) {
        return 473872185 + (n2 & 0xE3C148C6);
    }

    public static int method19420(int n2, int n3, AutoCrystal class419, Class682 class682) {
        int n4 = n2;
        return (0x151A7432 | ~n4) - ~n4;
    }

    public static int method19421(int n2, int n3, Class190 class190, Class268 class268) {
        int n4 = n2;
        return n4 & 0xC4637934 | 0x3B9C86CB & ~n4;
    }

    public static int method19422(int n2, int n3, Class31 class31, Class5457 class5457) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x1B94934C) & ~n4);
    }

    public static int method19423(int n2, int n3, Class3209 class3209, Class277 class277) {
        return 856843290 + (n2 & 0xCCED9BE5);
    }

    public static int method19424(int n2, int n3, MultiTask class410, Shulkerception class345) {
        return 168118745 + (n2 & 0xF5FAB626);
    }
}

