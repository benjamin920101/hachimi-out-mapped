/*
 * Decompiled with CFR 0.152.
 */
package mapped;

import mapped.Class1034;
import mapped.Class129;
import mapped.Class142;
import mapped.Class144;
import mapped.Class1463;
import mapped.Class1485;
import mapped.Class1660;
import mapped.Class170;
import mapped.Class1725;
import mapped.Class1804;
import mapped.Class232;
import mapped.Class234;
import mapped.Class268;
import mapped.Class270;
import mapped.AutoArmor;
import mapped.Search;
import mapped.Class3377;
import mapped.Class3414;
import mapped.Class3510;
import mapped.Class3556;
import mapped.Class3684;
import mapped.TickShift;
import mapped.NoJumpDelay;
import mapped.Notification;
import mapped.Class3878;
import mapped.NoGlitch;
import mapped.Replenish;
import mapped.Nuker;
import mapped.AntiCheat;
import mapped.Class5534;
import mapped.Class5582;
import mapped.Class5711;
import mapped.Class583;
import mapped.Class613;
import mapped.Class621;
import mapped.Class624;
import mapped.Class6286;
import mapped.Class681;
import mapped.Class688;
import mapped.Class711;
import mapped.Class92;

public class Class3990 {
    public static int method19576(int n2, int n3, Notification class382, Class624 class624) {
        int n4 = n2;
        return (0xB780476 | ~n4) - ~n4;
    }

    public static int method19577(int n2, int n3, Class621 class621, NoJumpDelay class381) {
        return 226454375 + (n2 & 0xF2809498);
    }

    public static int method19578(int n2, int n3, TickShift class371, Class234 class234) {
        int n4 = n2 ^ 0x14822841;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19579(int n2, int n3, Class3684 class3684, Class3414 class3414) {
        int n4 = n2 ^ 0x4200000;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19580(int n2, int n3, Class681 class681, Class688 class688) {
        return 946169525 + (n2 & 0xC79A994A);
    }

    public static int method19581(int n2, int n3, Class613 class613, Nuker class420) {
        int n4 = n3;
        return n4 + ((n2 ^ 0xB59011AD) & ~n4);
    }

    public static int method19582(int n2, int n3, Class3878 class3878, Class1660 class1660) {
        int n4 = n2 ^ 0x10440083;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19583(int n2, int n3, Class144 class144, Class142 class142) {
        return n2 ^ 0x2ABCC42C;
    }

    public static int method19584(int n2, int n3, Class1463 class1463, Class3556 class3556) {
        int n4 = n2 ^ 0x3B3FEFF3;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method19585(int n2, int n3, Class1034 class1034, Class5711 class5711) {
        int n4 = n2 ^ 0x41E4D14E;
        int n5 = n3;
        return (n4 | n5) & (~n5 | ~n4);
    }

    public static int method19586(int n2, int n3, Class268 class268, Class5582 class5582) {
        int n4 = n3;
        return n4 + ((n2 ^ 0x1AFE498C) & ~n4);
    }

    public static int method19587(int n2, int n3, Class3377 class3377, Class1804 class1804) {
        return 405391779 + (n2 & 0xE7D6365C);
    }

    public static int method19588(int n2, int n3, Replenish class408, Class270 class270) {
        int n4 = n2 ^ 0xC19E45BC;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19589(int n2, int n3, Class711 class711, Class5534 class5534) {
        int n4 = n2 ^ 0x49C63F30;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19590(int n2, int n3, Class6286 class6286, Class232 class232) {
        int n4 = n2 ^ 0;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19591(int n2, int n3, Class583 class583, NoGlitch class391) {
        int n4 = n2;
        return (0x1EE43714 | ~n4) - ~n4;
    }

    public static int method19592(int n2, int n3, Class5582 class5582, Class1485 class1485) {
        int n4 = n2;
        return (0x29D06650 | ~n4) - ~n4;
    }

    public static int method19593(int n2, int n3, AntiCheat class455, Class3510 class3510) {
        int n4 = n2;
        return (0xDE82293 | ~n4) - ~n4;
    }

    public static int method19594(int n2, int n3, Search class332, AutoArmor class279) {
        int n4 = n2 ^ 0x41AAA383;
        return (n3 | ~n4) - ~n4;
    }

    public static int method19595(int n2, int n3, Class129 class129, Class1725 class1725) {
        return 129317674 + (n2 & 0xF84AC4D5);
    }

    public static int method19596(int n2, int n3, Class170 class170, Class92 class92) {
        int n4 = n2;
        return (0x2C3F1234 | ~n4) - ~n4;
    }
}

